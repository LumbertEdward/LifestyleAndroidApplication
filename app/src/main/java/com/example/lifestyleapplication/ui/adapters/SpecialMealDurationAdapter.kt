...
open class SpecialMealDurationAdapter(val context: Context): RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return MyViewHolder(
            LayoutInflater.from(parent.context)
                .inflate(R.layout.selected__meal_duration_item, parent, false)
        )
    }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        myViewHolder.rel.setOnClickListener {
            general.getSpecialPlanDetails(list[position], mealPlan!!, condition!!)
        }
    }
    ...
    fun getDuration(lst: ArrayList<selectedday>, plan: String, cond: String){
        mealPlan = plan
        condition = cond
        for (i in lst){
            list.add(i)
